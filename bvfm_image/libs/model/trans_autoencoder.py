...
class Adaptor(nn.Module):
    def __init__(self, input_dim, tar_dim):
        ...
        self.fc3 = nn.Linear(4096, tar_dim)
...
class FlowEncoder(nn.Module):
    def __init__(self, d_model, N, head_num, d_ff, latten_size, dropout=0.1, last_norm=True):
        ...
        self.linear = nn.Linear(d_model, latten_size)
        self.norm = LayerNorm(latten_size) if last_norm else None
...
class MultiHeadAttentioin(nn.Module):
    def __init__(self, d_model, head_num, dropout=0.1, d_v=None):
        ...
        assert d_model % head_num == 0, f"d_model ({d_model}) must be divisible by head_num ({head_num})"
